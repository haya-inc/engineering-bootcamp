...
- Part: Part 4: 実行環境と運用
- 動画: https://www.youtube.com/watch?v=U9wpEuTV70Y
...
トレースは、1つのリクエストが複数の処理を通る流れを見るために使います。Webの入口、認可確認、DB問い合わせ、外部API呼び出しのうち、どこで時間がかかったかを追いやすくなります。新しく作るなら、まずOpenTelemetryやADOT、AWS Distro for OpenTelemetryを標準の集め方として考えます。AWS X-Rayは、集めたtraceを見たり受け取ったりする場所として理解すると、古いSDK前提の説明に引っ張られにくくなります。この章では本格構築ではなく、リクエストの通り道を見る考え方として扱います。
