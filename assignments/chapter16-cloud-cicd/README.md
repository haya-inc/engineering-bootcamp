...
- イメージタグは何を使うか。
- ECR repositoryは何か。
- App Runner serviceはstagingとproductionで分けるか。
...
- [ ] `aws-architecture-note.md` にGitHub Actions、ECR、App Runner、RDS、secrets、logsの役割がある
- [ ] stagingとproductionの違いが書かれている
- [ ] `ci-workflow-note.md` にPR時とmain merge時の検証内容がある
- [ ] `cd-workflow-note.md` にOIDC、IAMロール、イメージタグ、ECR push、App Runnerデプロイ、スモークテストがある
- [ ] 長期アクセスキーを標準にしていない
