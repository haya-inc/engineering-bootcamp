...
traceは、新しく作るなら、まずOpenTelemetryやADOT（AWS Distro for OpenTelemetry）を標準の集め方として考える。
ECS（Fargate）なら、ADOTをサイドカーコンテナとして入れ、アプリからtraceを送り出す形がよく使われる。
AWS X-Rayは、こうして集めたtraceを受け取り、見るための場所として理解するとよい。
古いX-Ray SDK前提の説明に引っ張られると、収集の主役がX-Rayだと誤解しやすいので注意する。
...
- **traces**：OpenTelemetryやADOTで集めたtraceを、X-Rayなどの画面で見る。request内の処理区間とlatencyを確認する。

CloudWatchは、metrics、alarms、dashboards、logsをまとめて扱える。
ここでも、traceを集める主役はOpenTelemetryやADOTであり、X-Rayはそのtraceを受け取って見る場所だと整理しておくと、古いX-Ray SDK前提の説明に引っ張られにくい。
