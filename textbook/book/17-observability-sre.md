...
第16章では、Docker imageをECRへ置き、App Runnerまたは講師指定の代替実行環境で動かし、CI/CDで変更を届ける流れを扱った。
しかし、リリースは仕事の終点ではない。
...
この章では、第16章で扱ったAWS App Runnerまたは講師指定の代替実行環境と、CloudWatch相当のログ、メトリクス確認を前提にする。
...
### App RunnerとCloudWatchでは、何を見るかを決めてから開く

第16章でApp Runnerへデプロイした場合、観測の入口は主にCloudWatchになる。
App Runnerは、service activity、CloudWatch Logsへ流れるログ、CloudWatch metrics、EventBridge event、CloudTrail、X-Ray tracingなどと接続できる。
ただし、第16章で確認したとおり、2026年6月時点でApp Runnerは新規顧客に開放されていない。
講師指定の代替実行環境を使う場合も、対応するログ、メトリクス、trace、deploy event、audit logの場所を書き換えて同じ考え方で確認する。
AWSの画面には多くのタブやグラフがあるため、目的を決めずに開くと迷いやすい。

まず見るのは、時間帯である。
問い合わせが来た時刻、deployした時刻、アラートが鳴った時刻を並べる。
次に、対象serviceとenvironmentを確認する。
stagingを見ているのかproductionを見ているのか。
直近のimage tagやrevisionは何か。
この確認を飛ばすと、正しいログを見ているつもりで別環境を見ていた、という初歩的だが重大なミスが起きる。

App RunnerとCloudWatchで見る観点は、次のように分ける。

- **service activity**：deploy、start、stop、health checkなど、service自体の出来事を見る。
- **application logs**：アプリが出したログを見る。request、error、duration、request_idを追う。
- **service metrics**：request count、response status、latency、CPU、memoryなどの推移を見る。
- **alarms**：人が対応すべき状態として設定した条件が発火しているかを見る。
- **traces**：X-Rayなどを使う場合、request内の処理区間とlatencyを見る。
...
「App Runner serviceが起動していること」は内部状態としては重要だが、利用者体験そのものではない。
...
- App RunnerやX-Rayの画面があるだけでtraceが取れていると思う。アプリ側のinstrumentationとspan設計を確認する。
...
- [AWS App Runner: Tracing for your application with X-Ray](https://docs.aws.amazon.com/apprunner/latest/dg/monitor-xray.html)
