...
第23章では、その改善PRをリリース候補として扱い、出してよいかを判断する。

本番リリース判定（Production Readiness Review、PRR）は、本番前に止める理由が残っていないかを確認する作業である。
...
不正なstatusは保存しない。
READMEも更新した。
この改善を、リリース候補として本番リリース判定にかける。

### この章を読み終えるとできるようになること

- リリース候補を、変更内容、非対象、環境、artifact、証拠で固定できる。
...
- 実施、条件付き実施、見送りを、証拠、blocker、accepted risk、follow-upで説明できる。

### 本番リリース判定はチェックリスト消化ではなく判断作業である

本番リリース判定をチェックリストの消化だと考えると、形だけになる。
...
この章の本番リリース判定も同じ姿勢で扱う。
誰かを責めるためではなく、リリース判断を説明可能にするために行う。

本番リリース判定の結論は、三つに分ける。
実施は、このまま進めてよいという判断である。
条件付き実施は、出せるが、追跡すべき残課題があるという判断である。
見送りは、今は出さず、先に直すべきblockerがあるという判断である。
...
これで確認対象が固定される。
本番リリース判定では、確認の深さより先に、確認対象の明確さが必要である。
artifactやcommit SHAが分からないまま本番リリース判定をすると、何を確認したのか後から追えない。
...
### 本番準備チェックリストは六つの見方で作る

本番リリース判定のチェックリストは、確認を機械的に終わらせるためではない。
見落としを減らすために使う。
この章では、六つの観点を使う。
...
| decision | condition |
| --- | --- |
| 実施 | high severity blockerがなく、主要フローとsmoke testが確認済み |
| 条件付き実施 | blockerはないが、期限つきで追うskipped/partialがある |
| 見送り | secret漏えい、権限外アクセス、既存データ消失、主要フロー失敗、戻し方不明などのblockerがある |
...
ただし、secret漏えい、権限外データアクセス、既存データ消失、主要フロー失敗は、見送り候補として扱う。

### セキュリティ確認は危ない使い方を防ぐ確認である

セキュリティ確認では、攻撃名を並べるより、今回の変更で危ない使い方ができないかを見る。
...
- 不正statusを保存できる場合は見送り。
- secretがREADMEやログに出ている場合は見送り。
- 権限外データが見える場合は見送り。
- 本番データや実在の個人情報を提出物やAI入力へ含めた場合は見送り。
...
### アクセシビリティ確認は使える人を減らさない確認である

アクセシビリティ確認では、主要操作を使える人を不必要に減らしていないかを見る。
...
### ログと監視の確認は問題時に見る場所を決める

ログと監視の確認では、問題が起きたときにどこを見るかを決める。
...
運用準備では、リリース前、リリース中、リリース後、問題時対応、rollback、cleanupを見る。
これは大げさな運用手順ではない。
...
- **environment**：
- **operator**：
- **進めてはいけない条件**：本番リリース判定のblockerが残っている、secret漏えいがある、rollback方針が未記入
...
| symptom | severity | check | action | 相談条件 |
| --- | --- | --- | --- | --- |
| 一覧が開かない | high | app log、browser Network | releaseを止める、revert検討 | 原因が15分以内に分からない |
| 既存ログが消えたように見える | high | migration note、DB/data、filter状態 | 見送りまたはrollback | 既存データ影響がある |
...
| --- | --- | --- |
| high severity blocker | none | 実施／条件付き実施が可能 |
| smoke test | pass | 実施／条件付き実施が可能 |
| skipped checks | documented | follow-upが必要 |
| accepted risks | documented | 条件付き実施の根拠 |
...
条件付き実施を選ぶなら、follow-upを追跡できる形にする。
「あとで見る」だけでは消える。
owner、期限、次の行動を書く。
...
```txt
第22章で作った改善PRについて、研修用の本番リリース判定を行います。
...
2. 実施 / 条件付き実施 / 見送り判断に必要な追加確認を提案してください。
3. smoke testを3項目に絞って提案してください。
4. リリース判断文書の不足をレビューしてください。
```
...
- 本番リリース判定をチェックリストの丸付けだと考える。確認結果から実施、条件付き実施、見送りを判断する。
- リリース候補を固定しない。何を出すのか、何を含めないのか、どの環境で見るのかを先に書く。
...
- AIの提案だけで実施にする。実行結果、差分、ログ、公式資料で確認する。
- 条件付き実施のfollow-upを追跡しない。owner、期限、次の行動を書く。
...
リリース候補を固定し、主要ユーザーフローと既知の問題を整理する。
...
最後に、リリース判断で実施、条件付き実施、見送りを証拠に基づいて決める。
