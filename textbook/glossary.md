...
- **具体例**：支援ステータス機能の `package.json` を見ると、`scripts.dev` が `node src/server.js`、`engines.node` が `>=26` であると分かり、起動方法と必要なバージョンが確認できる。
...
- **具体例**：支援ステータス機能はNode.js 26以上で動く。手元のNode.jsがその条件を満たすかを `node --version` で確かめてから起動する。
...
- **具体例**：支援ステータス機能はNode.js 26以上で動く。`node --version` の結果がその条件を満たすかを確かめてから `npm run dev` を実行する。
...
- **関連語**：ユースケース（第9章）、ドメインモデル（第9章）、DDD（第9章）
...
- **具体例**：支援ステータス機能のアプリ用に、`FROM node:26` から始め、`COPY package*.json ./` と `RUN npm ci` で依存関係を入れ、`CMD ["npm", "start"]` で起動するDockerfileを書く。
...
- **具体例**：支援ステータス機能のNode.jsアプリで `FROM node:26` を指定し、Node.js 26が入ったbase imageの上にアプリを載せる。
...
- **具体例**：支援ステータス機能を含む研修用Webアプリを、自分のPCではなくAWS上のECS（Fargate）で動かし、利用者がインターネット経由でアクセスできるようにする。
- **つまずきやすい点**：「クラウドに置けば勝手に安全・安価になる」と誤解しやすい。権限、secret、監視、cleanupは自分で設計する必要がある。
- **関連語**：AWS account（第16章）、region（第16章）、Amazon ECS（第16章）
...
- **くわしく**：CDを使うと、検証済みの変更をimage build、ECRへのpush、ECSへのdeploy（task definitionの更新とservice更新）、smoke testという同じ手順で環境へ届けられる。Continuous Deliveryは「いつでも出せる状態にする」、Continuous Deploymentは「自動で出すところまで進める」という違いがある。CDが成功したように見えても、deploy後の確認（ログやsmoke test）を省略しない。
- **具体例**：mainへmergeされたら、支援ステータス機能のimageをECRへpushし、staging用のECS service（Fargate）へdeployして、health endpointが200を返すか確認する。
...
- **具体例**：支援ステータス機能の変更が「PR → CI → review → merge → image build → ECR push → ECS deploy（task definition更新 + service更新）→ smoke test」という1本の流れを通って利用者へ届く。
...
- **具体例**：支援ステータス機能の新しいimageを、新しいtask definitionとしてstaging用のECS service（Fargate）へデプロイし、確認できたらproductionへデプロイする。
...
- **具体例**：支援ステータス機能のWebアプリを、AWSのECS（Fargate）、ECR、RDSという組み合わせで動かす。
...
- **具体例**：支援ステータス機能のECS（Fargate）とRDSを同じregionに置き、CD workflowでもそのregionを指定して接続する。
...
- **具体例**：支援ステータス機能のCD用IAM roleには「ECRへpushする」「ECS service（Fargate）を更新する（task definitionの登録とservice更新、task roleのPassRoleなど）」「必要なログを見る」だけを許可する。
...
- **具体例**：CD workflowが `role-to-assume` で支援ステータス機能のdeploy用roleを借り、ECRへのpushとECS service（Fargate）更新だけを実行できるようにする。なお、ECSがECRからimageをpullしログを出すために使うtask execution roleは、このdeploy用roleとは別物である。
...
- **くわしく**：ECRは、第15章のDockerfileから作ったcontainer imageを保管する場所である。CDではimageをbuildしてECRへpushし、ECS（Fargate）がそのimageを使ってアプリを実行する。あとから「どの変更を出したか」を追えるよう、imageには追跡できるtag（commit SHAなど）を付ける。`latest` だけに頼ると、どのimageがデプロイされたか分かりにくくなる。
- **具体例**：支援ステータス機能のimageを `app:<commit SHA>` のtagでECRへpushし、ECSのtask definitionからそのtagを指定して動かす。
- **つまずきやすい点**：`latest` tagだけで運用すると、デプロイ済みの中身を特定できなくなる。
- **関連語**：image tag（第16章）、Amazon ECS（第16章）、デプロイ（第16章）、artifact（第16章）
- **テキスト本文での登場箇所**：第16章「ECRは、どのimageを出したかを追う場所である」

### Amazon ECS
- **読み**：アマゾン イーシーエス（ファーゲート）（Amazon ECS / AWS Fargate）
- **一言で言うと**：container imageをFargate（サーバー管理をAWSに任せる起動タイプ）で動かす、AWSのコンテナ実行サービス。
- **くわしく**：Amazon ECS（Fargate 起動タイプ）は、EC2の管理をAWS側に任せながらcontainer imageを動かせるサービスである。本章では、ECRに置いたimageを使ってECSのcluster・task definition・service（Fargate 起動タイプ）を作る流れを想定する。ただし、使うimage、待ち受けるport、task definitionで渡す環境変数、secretの参照先、RDSへの接続、コンテナのhealth check、stagingとproductionの分け方などは自分たちで決める必要がある。imageが正しいだけでは足りない。最小公開ではFargateタスクにpublic IPを割り当てて直接公開できる（awsvpcモード + assignPublicIp=ENABLED + security groupでportを許可）が、これは学習用で本番非推奨であり、本番では正面にALBを置いてHTTPS終端・ヘルスチェック・複数タスクへの分散を担うのが定石である。
- **具体例**：支援ステータス機能のimageを使い、stagingとproductionでそれぞれECS service（Fargate）を作り、task definitionで必要な環境変数とsecretを渡す。
- **つまずきやすい点**：managedでも設計は不要にならない。port、secret、DB接続、ログまで含めて確認する。
- **関連語**：ECR（第16章）、RDS（第16章）、managed service（第16章）、CloudWatch Logs（第16章）
- **公式ドキュメント**：[Amazon ECS とは](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/Welcome.html)、[AWS Fargate（ECS 上）](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/AWS_Fargate.html)、[task execution role](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_execution_IAM_role.html)
- **テキスト本文での登場箇所**：第16章「ECS（Fargate）は、Webアプリを動かす場所である」

### managed service
- **読み**：マネージド サービス（managed service）
- **一言で言うと**：サーバー運用の一部をクラウド側に任せられるサービス。
- **くわしく**：managed serviceを使うと、OSの管理やスケーリングなど運用作業の一部をクラウドに任せられる。ECS（Fargate）やRDSはその例である。Fargateはサーバー（EC2）の管理をAWSに任せられる点でmanagedといえる。運用の手間は減るが、設定や設計の責任までなくなるわけではない。どの環境変数を渡すか、どこにsecretを置くか、どう接続するかは自分たちで決める。
- **具体例**：支援ステータス機能のWebアプリ実行をECS（Fargate）に、DB運用をRDSに任せ、アプリ側の設定と接続だけに集中する。
- **つまずきやすい点**：「managedだから何も考えなくてよい」と誤解しやすい。設定・接続・監視は自分の責任。
- **関連語**：Amazon ECS（第16章）、RDS（第16章）、クラウド（第16章）
- **テキスト本文での登場箇所**：第16章「ECS（Fargate）は、Webアプリを動かす場所である」
...
- **具体例**：支援ステータス機能のECS（Fargate）が、staging用RDSとproduction用RDSにそれぞれ接続し、DBは環境ごとに分ける。
- **つまずきやすい点**：「接続できない」の一言では原因が分からない。どの段階（接続先・認証・network・DB準備状態）で失敗しているかを切り分ける。
- **関連語**：Amazon ECS（第16章）、network（第16章）、staging（第16章）、production（第16章）
- **テキスト本文での登場箇所**：第16章「RDSは、DBを置く場所である」

### network
- **読み**：ネットワーク（network）
- **一言で言うと**：どことどこが通信できるかを決める境界。
- **くわしく**：networkは、AWSのリソース同士やリソースと外部が通信できる範囲を決める。ECS（Fargate）はawsvpcモードで各タスクがENIを持ちVPCに直接所属するので、RDSへ接続するには、タスクを置くsubnetとsecurity group、そしてRDS側のsecurity groupがECSタスクのsecurity group（PostgreSQLのport）を許可しているかが必要になる。接続先情報や認証情報が正しくても、networkが通っていなければつながらない。RDS接続のトラブルでは、networkとsecurity groupも確認対象に含める。
- **具体例**：支援ステータス機能のECS（Fargate）からRDSへ接続できないとき、認証情報だけでなく、タスクのsubnet/security groupとRDS側のsecurity groupが通っているかを確認する。
...
- **くわしく**：Secrets Managerは、`.env` のような秘密情報をproductionへそのまま持ち込まず、安全に管理・参照するための仕組みである。DB password、SESSION_SECRET、外部API tokenなどを置き、ECS（Fargate）などから参照する。文書には値そのものではなく、置き場所、使う場面、確認手順を書く。secretを変更したら、どの環境へ反映したかを確認する。
- **具体例**：支援ステータス機能の `DB_PASSWORD` と `SESSION_SECRET` をSecrets Managerに置き、ECSのtask definitionから参照する。値はPRやログに貼らない。
...
- **くわしく**：環境変数は、DB接続先、app port、ログレベルなど、local・staging・productionで変わる値を外から渡すために使う。クラウドではECSのtask definitionで定義する環境変数として設定できる。通常の設定値はそのまま渡してよいが、secretに当たるものはSecrets ManagerやParameter Storeから参照する。同じimageを使い、環境ごとに変わる値を外から渡すと、環境差分を説明しやすくなる。
- **具体例**：支援ステータス機能で、`APP_ENV` や `LOG_LEVEL` をECSのtask definitionで定義する環境変数として渡し、`DB_PASSWORD` はsecret管理から参照する。
...
- **関連語**：Amazon ECS（第16章）、smoke test（第16章）、デプロイ（第16章）
...
- **具体例**：支援ステータス機能で、`ci` workflowがtestとbuildを、`deploy-staging` workflowがECR pushとECS deploy（task definition更新 + service更新）を担当する。
...
- **くわしく**：artifactは、CI/CDの過程で生成される成果物のことで、本章ではbuildしたcontainer imageが代表的なartifactにあたる。一度buildしたartifactをECRへ保存し、ECS（Fargate）がそれを使ってデプロイする。追跡できるtagを付けておくと、どのartifactをどの環境へ出したかを結びつけやすい。検証済みのartifactを使うことで、環境ごとに作り直さずに済む。
...
- **くわしく**：image tagは、ECRに置いたimageを識別するためのラベルである。`latest` だけに頼ると、どの変更がデプロイされたか分かりにくくなる。commit SHAやrelease tagを使うと、Gitの変更、GitHub Actionsの実行、ECRのimage、ECSのdeployを一本につなげられる。release runbookには、今回出したimage tagと直前に動いていたimage tagを残し、rollbackの判断材料にする。
...
- **くわしく**：stagingは、localより本番に近く、productionより利用者への影響が小さい環境である。staging用のECS service（Fargate）、RDS、secret、domain、logsを用意し、productionへ出す前に動作を確認する。環境を分ける理由は手順を複雑にするためではなく、利用者影響、データ、secret、権限、確認の厳しさが違うからである。
...
- **くわしく**：productionは、実利用者が使う環境なので、デプロイ時の確認がもっとも厳しくなる。デプロイ前に、CI結果、変更内容、DBへの影響、secret変更、rollback候補を確認する。必要ならmanual approvalを入れる。production用のECS service（Fargate）、RDS、secret、domainを用意し、本番DBに練習用データを入れないなど、データ保護も意識する。
...
- **関連語**：staging（第16章）、rollback（第16章）、smoke test（第16章）
- **テキスト本文での登場箇所**：第16章「stagingとproductionを分ける」
...
- **関連語**：rollback（第16章）、smoke test（第16章）、image tag（第16章）
- **テキスト本文での登場箇所**：第16章「rollbackは、失敗してから考えると遅い」
...
- **くわしく**：ECS（Fargate）などで動くアプリのrequestを、処理区間（span）に分けて追えるようにするAWSのトレーシング機能である。trace上のlatencyを見て、request内のどの処理が遅いかを切り分けるのに使う。本章では画面操作を暗記する対象にはしないが、CloudWatchのlogs/metrics/alarmsに加えて、tracesの入口としてX-Rayがあると押さえておくとよい。
...
- **テキスト本文での登場箇所**：第17章「ECS（Fargate）とCloudWatchでは、何を見るかを決めてから開く」
...
- **関連語**：metadata filter（第18章）、RAG（第18章）
- **テキスト本文での登場箇所**：第18章「RAGは、検索して読ませる設計である」
...
- **関連語**：tool use（第18章）、human-in-the-loop（第18章）
...
- **関連語**：human-in-the-loop（第18章）、tool use（第18章）
...
- **関連語**：OWASP Top 10 for LLM（第18章）、評価（第18章）
- **テキスト本文での登場箇所**：第18章「AI利用リスクは、使わない理由ではなく設計対象である」

### OWASP Top 10 for LLM
- **読み**：オワスプトップテンフォーエルエルエム（OWASP Top 10 for Large Language Model Applications）
- **一言で言うと**：LLMアプリで起きやすい代表的なリスクを整理した一覧。
- **くわしく**：OWASP Top 10 for LLMは、OWASPが公開する、LLMや生成AIアプリで起きやすいリスクを整理した一覧である。現行版（2025年版）には、prompt injection、sensitive information disclosure、excessive agency、misinformation（overrelianceはこのmisinformationに含まれる）などが挙げられている。AI機能を作るとき、どこにリスクがあるかを見落とさないチェックの出発点になる。リスク対策は、最後にチェックリストを足すだけでは弱く、prompt、context、tool権限、停止条件、評価表、ログ設計へ分散して入れる。
...
- **関連語**：NIST AI RMF（第18章）、prompt injection（第18章）
- **テキスト本文での登場箇所**：第18章「AI利用リスクは、使わない理由ではなく設計対象である」
...
- **関連語**：AI coding agent（第19章）、Claude Code（第19章）
...
- **関連語**：AI coding agent（第19章）、Claude Code（第19章）、AIコーディング（第19章）
...
- **関連語**：permission（第19章）、sandbox（第19章）
- **テキスト本文での登場箇所**：第19章「permissions、sandbox、approvalで、読む、書く、実行するを分ける」
...
この章では、AIコーディングの作業で生まれた情報を、チームが使える技術文書へ変える。README、ADR、runbook を読者と目的で使い分け、AIで整えた文章を人間が確認する流れを学ぶ。ここでは、研修の外でも通用する一般的な専門用語を解説する。
